            ...
            <CardContent>
                <form onSubmit={submit} className="space-y-6">
                    <div className="grid gap-6 sm:grid-cols-2">
                        ...
                        <div className="space-y-2"><InputLabel htmlFor="current_plate" value="Plat Nomor" /><TextInput id="current_plate" className="block w-full" value={form.data.current_plate} onChange={(e) => form.setData('current_plate', e.target.value.toUpperCase())} required /><InputError message={form.errors.current_plate} /></div>
                        <div className="space-y-2"><InputLabel htmlFor="type" value="Tipe" /><TextInput id="type" className="block w-full" value={form.data.type} onChange={(e) => form.setData('type', e.target.value)} required /><InputError message={form.errors.type} /></div>
                    ...
                    <div className="flex items-center gap-3"><PrimaryButton disabled={form.processing}>Simpan</PrimaryButton><Link href={route('units.index')} className="text-sm text-muted-foreground hover:text-foreground">Batal</Link></div>
                </form>
            </CardContent>
        </Card>
    );
