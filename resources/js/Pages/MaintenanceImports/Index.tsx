...
export default function Index({ imports, preview }: PageProps<{ imports: MaintenanceImport[]; preview?: Preview }>) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-foreground">Import Data Existing</h2>}>
            <Head title="Import Data Existing" />
            <div className="py-10">
                            ...
                            <form onSubmit={submitPreview} className="space-y-6">
                                <div className="grid gap-6 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        <InputLabel value="Jenis Import" />
                                        <Select value={form.data.type} onValueChange={(value) => form.setData('type', value as 'units' | 'unit_plannings')}>
                                            <SelectTrigger><SelectValue /></SelectTrigger>
                                            <SelectContent>
                                                <SelectItem value="units">Import Units</SelectItem>
                                                <SelectItem value="unit_plannings">Import Unit Plannings</SelectItem>
                                            </SelectContent>
                                        </Select>
                                        <InputError message={form.errors.type} />
                                    </div>
                                    <div className="space-y-2">
                                        <InputLabel value="File CSV / Excel" />
                                        <input
                                            type="file"
                                            accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                                            onChange={(event) => form.setData('file', event.target.files?.[0] ?? null)}
                                            className="block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground file:text-foreground dark:bg-input/30"
                                        />
                                        <InputError message={form.errors.file} />
                                    </div>
                                </div>
                                <PrimaryButton disabled={form.processing}>Preview File</PrimaryButton>
                            </form>
                        </CardContent>
                    </Card>
                        ...
                        <Card>
                            <CardContent>
                                ...
                                <div className="mt-6 overflow-x-auto">
                                    <Table>
                                        <TableHeader>
                                            <TableRow><TableHead>Line</TableHead><TableHead>Status</TableHead><TableHead>Catatan</TableHead><TableHead>Data</TableHead></TableRow>
                                        </TableHeader>
                                        <TableBody>
