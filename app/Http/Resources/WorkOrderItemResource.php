...
class WorkOrderItemResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'action' => $this->action,
            'status' => $this->status,
            'reason' => ($this->unitPlanning?->isBaselineMissing() ?? true) ? null : $this->reason,
            'historical_reason' => ($this->unitPlanning?->isBaselineMissing() ?? true) ? $this->reason : null,
            'notes' => $this->notes,
            'baseline_last_done_km' => $this->baseline_last_done_km,
            'baseline_last_done_date' => $this->baseline_last_done_date?->toDateString(),
            'new_due_km' => $this->new_due_km,
            'new_due_date' => $this->new_due_date?->toDateString(),
            'effective_due_km' => $this->new_due_km ?? $this->unitPlanning?->next_due_km,
