...
class UnitHistoryController extends Controller
{
    public function show(Request $request, Unit $unit): Response
    {
        ...
        return Inertia::render('Units/History', [
            'history' => UnitHistoryResource::make([
                ...
                'replacements' => $this->itemsFor($unit, ['complete'])->where('action', 'replace')->latest('completed_date')->get()->map(fn (WorkOrderItem $item): array => $this->historyItem($item))->values(),
                'plate_histories' => $unit->plateHistories()->latest('active_from')->get()->map(fn ($history): array => [
                    'id' => $history->id,
                ...
                'blocked_breakdowns' => $this->itemsFor($unit, ['blocked', 'breakdown'])->latest()->get()->map(fn (WorkOrderItem $item): array => $this->historyItem($item))->values(),
                'postpones' => $this->itemsFor($unit, ['postponed'])->latest()->get()->map(fn (WorkOrderItem $item): array => $this->historyItem($item))->values(),
            ])->resolve(),
        ]);
    }
    ...
    private function abortIfCannotAccessUnit(Request $request, Unit $unit): void
    {
        ...
        if ($user->hasRole(UserRole::AdminSite) && $unit->site_id === $user->site_id) {
            return;
        }
